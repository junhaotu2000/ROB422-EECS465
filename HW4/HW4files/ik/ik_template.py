...
def main():
    ...
    # initialize PyBullet
    connect(use_gui=True, shadows=False) # 初始化PyBullet仿真环境，包括开启图形用户界面（GUI）并关闭阴影以提高性能。
    
    # load robot
    with HideOutput(): # 其目的可能是为了抑制在其代码块内生成的控制台输出
    ...
    joint_names =['l_shoulder_pan_joint','l_shoulder_lift_joint','l_upper_arm_roll_joint', \
        'l_elbow_flex_joint','l_forearm_roll_joint','l_wrist_flex_joint','l_wrist_roll_joint'] # 定义机器人的活动关节名称。
    joint_idx = [joint_from_name(robot, jn) for jn in joint_names] # 定义机器人手臂的活动关节并获取这些关节的索引。
    
    # intial config
    q_arr = np.zeros((1, len(joint_idx))) # 初始化关节位置配置，此时所有关节均在零位置。
    ...
    wait_if_gui() # 如果启用了图形用户界面(GUI)，该命令会使脚本暂停，直到用户采取某些行动（例如关闭UI窗口或点击停止按钮），这样用户就有时间观察和分析仿真结果。
    disconnect() # 断开当前的PyBullet会话，这通常意味着关闭仿真环境并释放相关资源。如果有GUI界面，它也会随之关闭。
